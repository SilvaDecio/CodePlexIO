...
namespace Decio.Joysticks.Analogic
{
    class AnalogicJoypad
    {
        ...
        public AnalogicJoypad(ContentManager Content , Vector2 position)
        {
            ...
            MiniButton = new MiniPad(Content.Load<Texture2D>("MiniPad"), Position);

            MediumButton = new MediumPad(Content.Load<Texture2D>("MediumPad"), Position);

            BigButton = new BigPad(Content.Load<Texture2D>("BigPad"), Position);
